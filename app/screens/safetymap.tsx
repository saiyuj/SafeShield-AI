...
const DANGER_ZONES = [
  { id: 1, name: 'Dark Alley - MG Road', type: 'Poorly Lit', reports: 12, color: '#FF3B30', icon: 'moon-outline' },
        ...
        {activeTab === 'danger' && (
          <View>
            <View style={s.alertBanner}>
              ...
              <Text style={s.alertBannerText}>{DANGER_ZONES.length} danger zones reported near you</Text>
            </View>
            {DANGER_ZONES.map((zone) => (
              <View key={zone.id} style={[s.zoneCard, { borderColor: zone.color + '44' }]}>
                ...
                <Ionicons name="alert-circle-outline" size={20} color={zone.color} />
              </View>
            ))}
          </View>
        )}
        ...
        {activeTab === 'safe' && (
          <View>
            <View style={s.safeBanner}>
              ...
              <Text style={s.safeBannerText}>{SAFE_ZONES.length} safe zones near you</Text>
            </View>
            {SAFE_ZONES.map((zone) => (
              <View key={zone.id} style={[s.zoneCard, { borderColor: zone.color + '44' }]}>
                <View style={[s.zoneIcon, { backgroundColor: zone.color + '22' }]}>
                ...
                <TouchableOpacity style={[s.navigateBtn, { backgroundColor: zone.color }]} onPress={() => Alert.alert('Navigation', `Opening directions to ${zone.name}`)}>
        ...
        {activeTab === 'report' && (
          <View>
            <View style={s.reportHero}>
              ...
              <Text style={s.reportSub}>Help protect your community by marking unsafe areas</Text>
            </View>
            <View style={s.reportCard}>
              <Text style={s.reportLabel}>SELECT DANGER TYPE</Text>
              <View style={s.dangerTypeGrid}>
                {dangerTypes.map((type, i) => (
                  <TouchableOpacity key={i} style={[s.dangerTypeBtn, reportType === type && s.dangerTypeBtnActive]} onPress={() => setReportType(type)}>
                    <Text style={[s.dangerTypeBtnText, reportType === type && s.dangerTypeBtnTextActive]}>{type}</Text>
                  </TouchableOpacity>
                ))}
              </View>
              <TouchableOpacity style={s.submitBtn} onPress={reportDangerZone} activeOpacity={0.8}>
...
const s = StyleSheet.create({
  ...
  navigateBtn: { width:36, height:36, borderRadius:18, justifyContent:'center', alignItems:'center' },
  ...
  reportSub: { color:'#555', fontSize:13, textAlign:'center' },
  reportCard: { backgroundColor:'#1C1C1E', borderRadius:20, padding:20, marginBottom:16, borderWidth:1, borderColor:'#2C2C2E' },
  reportLabel: { color:'#555', fontSize:11, fontWeight:'700', letterSpacing:1.5, marginBottom:12 },
  dangerTypeGrid: { flexDirection:'row', flexWrap:'wrap', gap:8, marginBottom:20 },
  dangerTypeBtn: { backgroundColor:'#111111', borderRadius:10, paddingHorizontal:12, paddingVertical:8, borderWidth:1, borderColor:'#2C2C2E' },
  dangerTypeBtnActive: { backgroundColor:'#FF3B30', borderColor:'#FF3B30' },
  dangerTypeBtnText: { color:'#555', fontSize:12, fontWeight:'600' },
  dangerTypeBtnTextActive: { color:'white' },
